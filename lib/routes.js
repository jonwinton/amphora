...
function addSiteLocals(site) {
  return function (req, res, next) {
    ...
    //res.locals.pageUri = _.find(getPagesUris(req));
/*    if (req.uri.indexOf('/pages/') > -1) {
      res.locals.pageUri = req.uri;
    }
*/
    res.locals.pageUri = req.uri;
...
function addHost(options) {
  //console.log('what are our options in routes', options);
